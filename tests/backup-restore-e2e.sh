...
require_runtime
[[ -f "$env_file" ]] || { echo "ERROR: Deploy Packet Expert before running recovery acceptance." >&2; exit 3; }
...
cleanup() {
  docker volume rm "$ollama_test_volume" "$webui_test_volume" >/dev/null 2>&1 || true
...
echo "PASS: consistent backup checksums and non-overwriting restore were verified."
