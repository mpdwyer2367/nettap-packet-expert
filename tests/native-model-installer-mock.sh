...
case "${1:-}" in
  list)
    ...
    if [[ -f "${NETTAP_TEST_STATE_DIR}/created" ]]; then
      printf '%s\n' 'nettap-ai:0.4.0-rc.1 current 6.6 GB now'
    fi
  ...
  pull)
    ...
    ;;
  create)
    [[ "${2:-}" == "nettap-ai:0.4.0-rc.1" ]]
    [[ "${3:-}" == "-f" && -f "${4:-}" ]]
    : >"${NETTAP_TEST_STATE_DIR}/created"
    ;;
  show)
    [[ "${2:-}" == "--modelfile" ]]
    [[ "${3:-}" == "nettap-ai:0.4.0-rc.1" ]]
...
grep -Fq 'PASS: nettap-ai:0.4.0-rc.1 is saved in the active Ollama store.' \
  "$test_dir/output.txt"
