...
case "${1:-}" in
  list)
    ...
    if [[ -f "${NETTAP_TEST_STATE_DIR}/created" ]]; then
      printf '%s\n' 'nettap-ai:0.3.0-rc.5 current 4.7 GB now'
    fi
  ...
  pull)
    ...
    ;;
  create)
    [[ "${2:-}" == "nettap-ai:0.3.0-rc.5" ]]
    [[ "${3:-}" == "-f" && -f "${4:-}" ]]
    : >"${NETTAP_TEST_STATE_DIR}/created"
    ;;
  show)
    [[ "${2:-}" == "--modelfile" ]]
    [[ "${3:-}" == "nettap-ai:0.3.0-rc.5" ]]
...
grep -Fq 'PASS: nettap-ai:0.3.0-rc.5 is saved in the active Ollama store.' \
  "$test_dir/output.txt"
