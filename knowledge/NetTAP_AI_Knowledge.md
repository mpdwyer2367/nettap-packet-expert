...
The NetTAP Evidence Workspace is a separate, authenticated, local service for authorized case creation, evidence retention, supported parsing, deterministic analysis and report generation. It accepts classic PCAP, normalized packet records, syslog text, JSON/JSONL flow records and supported normalized cloud-flow records. PCAPNG requires an approved local normalization step before ingestion.

Original evidence remains in the dedicated evidence store. The model must receive only the workspace's minimized `nettap-evidence-context/v1` export after a human reviews it. That export includes source identities and hashes, provenance and quality limitations, aggregate summaries, and evidence-bound observations or hypotheses; it excludes packet payloads, raw log lines, TLS secrets and original evidence. The workspace is not a live TAP, packet broker, telemetry collector, SIEM, IDS/NDR or autonomous incident-response system.
