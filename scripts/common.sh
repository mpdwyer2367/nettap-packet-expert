...
initialize_env() {
  ...
  chmod 0600 "$env_file"
  ensure_env_default RELEASE_VERSION "0.4.0-rc.1"
  ...
  ensure_env_default NETTAP_AI_MODEL "nettap-ai:0.4.0-rc.1"
  ensure_env_default MODEL_NAME "nettap-ai:0.4.0-rc.1"
  ...
  ensure_env_default CONSOLE_IMAGE "nettap/analyst-console:0.4.0-rc.1"
  ensure_env_default CONSOLE_NODE_IMAGE "node:22.18.0-bookworm-slim@sha256:752ea8a2f758c34002a0461bd9f1cee4f9a3c36d48494586f60ffce1fc708e0e"
  ensure_env_default CONSOLE_HTTPS_PORT "9443"
  ensure_env_default CONSOLE_CPUS "2"
  ensure_env_default CONSOLE_MEMORY "2g"
  ensure_env_default SUPABASE_URL "CHANGE_ME"
  ensure_env_default SUPABASE_WSS_URL "CHANGE_ME"
  ensure_env_default SUPABASE_PUBLISHABLE_KEY "CHANGE_ME"
  ensure_env_default SUPABASE_SERVICE_ROLE_KEY "CHANGE_ME"
  ...
  if grep -Eq '^RELEASE_VERSION=(0\.2\.0-rc\.1|0\.3\.0-rc\.[1-4]|0\.4\.0-qwen35-9b-rc\.1)$' "$env_file"; then
    set_env_value RELEASE_VERSION "0.4.0-rc.1"
  ...
  if grep -Eq '^MODEL_NAME=(nettap-packet-expert:(0\.1\.0-rc\.8|0\.2\.0-rc\.1|0\.3\.0-rc\.1)|nettap-ai:(latest|0\.3\.0-rc\.[1-4]|0\.4\.0-qwen35-9b-rc\.1))$' "$env_file"; then
    set_env_value MODEL_NAME "nettap-ai:0.4.0-rc.1"
  fi
  if grep -Eq '^NETTAP_AI_MODEL=(nettap-packet-expert:[^[:space:]]+|nettap-ai:(latest|0\.3\.0-rc\.[1-4]|0\.4\.0-qwen35-9b-rc\.1))$' "$env_file"; then
    set_env_value NETTAP_AI_MODEL "nettap-ai:0.4.0-rc.1"
  fi
  if grep -Eq '^BASE_MODEL=(qwen2\.5:7b-instruct-q4_K_M|qwen3\.5:9b)$' "$env_file"; then
    set_env_value BASE_MODEL "qwen3.5:9b-q4_K_M"
  fi
  if grep -Eq '^EXPECTED_BASE_MODEL_ID=(845dbda0ea48|6488c96fa5fa)$' "$env_file"; then
    set_env_value EXPECTED_BASE_MODEL_ID "6488c96fa5fa"
  fi
  if grep -q '^OLLAMA_MEMORY=8g$' "$env_file"; then
    set_env_value OLLAMA_MEMORY "12g"
  fi
