...
initialize_env() {
  ...
  chmod 0600 "$env_file"
  ensure_env_default RELEASE_VERSION "0.3.0-rc.4"
  ...
  ensure_env_default NETTAP_AI_MODEL "nettap-ai:0.3.0-rc.4"
  ensure_env_default MODEL_NAME "nettap-ai:0.3.0-rc.4"
  ...
  if grep -Eq '^RELEASE_VERSION=(0\.2\.0-rc\.1|0\.3\.0-rc\.[123])$' "$env_file"; then
    set_env_value RELEASE_VERSION "0.3.0-rc.4"
  ...
  if grep -Eq '^MODEL_NAME=(nettap-packet-expert:(0\.1\.0-rc\.8|0\.2\.0-rc\.1|0\.3\.0-rc\.1)|nettap-ai:(latest|0\.3\.0-rc\.[123]))$' "$env_file"; then
    set_env_value MODEL_NAME "nettap-ai:0.3.0-rc.4"
  fi
  if grep -Eq '^NETTAP_AI_MODEL=(nettap-packet-expert:[^[:space:]]+|nettap-ai:(latest|0\.3\.0-rc\.[123]))$' "$env_file"; then
    set_env_value NETTAP_AI_MODEL "nettap-ai:0.3.0-rc.4"
  fi
