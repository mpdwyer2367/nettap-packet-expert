...
[[ "${1:-}" == "--confirm" && $# -eq 1 ]] || {
  echo "Usage: ./scripts/recover-admin.sh --confirm" >&2
  echo "This operation backs up the Open WebUI database, resets the single administrator to the canonical identity, and invalidates existing sessions." >&2
  exit 2
}
...
"${selected[@]}" exec -T open-webui python - "$container_backup" <<'PY'
import sqlite3
import sys
...
if ! printf '%s\n' "$recovery_password" | \
  "${selected[@]}" exec -T open-webui python -c "$reset_python"; then
