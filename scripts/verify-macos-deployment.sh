...
[[ "$nettap_model" == "nettap-ai:0.3.0-rc.8" ]] || fail "Unexpected model identity: $nettap_model"
...
for retired_port in 3000 3001 3200; do
  if curl --silent --show-error --max-time 2 "http://127.0.0.1:${retired_port}/" >/dev/null 2>&1; then
    fail "Retired loopback port ${retired_port} is still serving an HTTP endpoint."
  fi
done
echo "PASS: retired loopback ports 3000, 3001 and 3200 are closed"
...
assert p['release_version'] == '0.3.0-rc.8'
