...
$content = $content -replace '(?m)^RELEASE_VERSION=(0\.2\.0-rc\.1|0\.3\.0-rc\.[1-4]|0\.4\.0-qwen35-9b-rc\.1)$', 'RELEASE_VERSION=0.4.0-rc.1'
$content = $content -replace '(?m)^MODEL_NAME=(nettap-packet-expert:(0\.1\.0-rc\.8|0\.2\.0-rc\.1|0\.3\.0-rc\.1)|nettap-ai:(latest|0\.3\.0-rc\.[1-4]|0\.4\.0-qwen35-9b-rc\.1))$', 'MODEL_NAME=nettap-ai:0.4.0-rc.1'
$content = $content -replace '(?m)^NETTAP_AI_MODEL=(nettap-packet-expert:[^\s]+|nettap-ai:(latest|0\.3\.0-rc\.[1-4]|0\.4\.0-qwen35-9b-rc\.1))$', 'NETTAP_AI_MODEL=nettap-ai:0.4.0-rc.1'
$content = $content -replace '(?m)^BASE_MODEL=(qwen2\.5:7b-instruct-q4_K_M|qwen3\.5:9b)$', 'BASE_MODEL=qwen3.5:9b-q4_K_M'
$content = $content -replace '(?m)^EXPECTED_BASE_MODEL_ID=(845dbda0ea48|6488c96fa5fa)$', 'EXPECTED_BASE_MODEL_ID=6488c96fa5fa'
$content = $content -replace '(?m)^OLLAMA_MEMORY=8g$', 'OLLAMA_MEMORY=12g'
