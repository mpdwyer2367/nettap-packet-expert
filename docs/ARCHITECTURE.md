...
NetTAP Network Observability & Packet Analysis has one authenticated user interface and one managed assistant. Network-design, observability, troubleshooting, packet-analysis, security and forensic guidance are combined without duplicating model weights.
...
flowchart TD
    ...
    F -->|"PCAP, logs, flow records"| E["Internal evidence parser"]
    F -->|"Validated network images"| O
...
| Ollama | Multimodal Qwen3.5 9B base and `nettap-ai:0.3.0-rc.8` manifest | Docker backend only |
...
2. The managed Filter resolves the server-side upload and verifies the extension. Evidence records go to the private deterministic service; supported network images are signature-validated and encoded for the multimodal model.
...
4. Only minimized evidence context—IDs, hashes, quality warnings, observations, hypotheses and limitations—is injected for parsed records. Network images are the exception: their pixels are supplied to the local multimodal model, never to an external service.
...
| PNG/JPEG/WebP | Signature-validated local visual analysis of network diagrams and screenshots; maximum four images and 10 MB per image |
