...
Release `0.3.0-rc.1` is an integration candidate for one single-node, single-customer Docker deployment. It is not a certified GA appliance until every commercial release gate passes.
...
| Network assistant | `nettap-network-visibility:0.3.0-rc.1` |
| Packet assistant | `nettap-packet-expert:0.3.0-rc.1` |
...
The Ollama policies load when the assistant models are created. Supplemental Markdown knowledge requires a controlled Open WebUI import and matching Workspace Model attachment. Git changes do not update deployed knowledge.
...
4. Rebuild both assistant models.
5. Run static, behavioral, runtime, storage, launcher, knowledge-isolation, backup, restore, and rollback checks.
...
### One assistant is missing

Run `./scripts/nettap-ai update-models --confirm`, inspect `ollama list`, and rerun the behavior tests. Do not create an unversioned substitute model from the UI.
