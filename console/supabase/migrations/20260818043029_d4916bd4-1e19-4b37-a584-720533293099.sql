...
BEGIN
  ...
  LOOP
    ...
    SELECT b.dataset_id, b.user_id, b.bucket_ts, b.src_ip, b.dst_ip, b.src_port, b.dst_port, b.protocol,
           b.app_protocol, b.service, d.vantage, b.packets, b.bytes, 1, b.risk_tags
    FROM (
      SELECT p.dataset_id, p.user_id, date_trunc('minute', p.ts) AS bucket_ts,
             p.src_ip, p.dst_ip, p.src_port, p.dst_port, p.protocol,
             min(p.app_protocol) AS app_protocol, min(p.service) AS service,
             count(*)::bigint AS packets, COALESCE(sum(p.length),0)::bigint AS bytes,
             COALESCE((SELECT array_agg(DISTINCT tg) FROM unnest(array_remove(array_agg(p.risk_tags::text), NULL)) raw_tag,
                       unnest(raw_tag::text[]) tg), '{}'::text[]) AS risk_tags
