#!/usr/bin/env bash
# NetTAP standalone live-capture agent for Linux.
...
dumpcap -D || { echo "ERROR: Configure non-root dumpcap permissions." >&2; exit 1; }
printf "Enter the interface name or number shown above: "; read -r IFACE
[ -n "$IFACE" ] || { echo "ERROR: A capture interface is required." >&2; exit 1; }
...
SPOOL_DIR="$(mktemp -d "${TMPDIR:-/tmp}/nettap-live.XXXXXX")"; chmod 700 "$SPOOL_DIR"
